"""
High-Speed Empirical Backtest for ADX + DI Filter Impact Analysis

Datasets:
1. NIFTY 90-Days 5-Minute Historical Data (data/historical/nifty_5min.csv) - 18,450 5M bars
2. NIFTY 90-Days 1-Minute Historical Data (data/historical/nifty_90days.csv) - Recent 15,000 1M bars (~40 full trading sessions)
"""
import sys
import json
...
def run_adx_comparison(df: pd.DataFrame, dataset_label: str):
    ...
    print(f"\n=======================================================")
    print(f"RUNNING EMPIRICAL BACKTEST: {dataset_label}")
    print(f"Candles: {len(df):,} | From: {df['timestamp'].min()} to {df['timestamp'].max()}")
    print(f"=======================================================")

    results = {}

    for min_adx in [0, 20, 25]:
        mode_label = "Baseline (No ADX Gate)" if min_adx == 0 else f"ADX >= {min_adx} (+DI / -DI Gate)"
        print(f"\nEvaluating: {mode_label}...")

        dm = DataManager(window_size=300, underlying="NIFTY")
        trader = PaperTrader(
        ...
        for i, row in enumerate(df.itertuples()):
            if i % 3000 == 0:
                print(f"  Processed {i:,} / {total_rows:,} bars...")
        ...
        for o in trader.get_positions():
            trader.close_position(o.order_id, o.entry_price, timestamp=df['timestamp'].iloc[-1], reason="END_OF_DATA")
    ...
    print("\n" + "=" * 90)
    print(f"BENCHMARK RESULTS: {dataset_label}")
    print("=" * 90)
...
if __name__ == "__main__":
    raw_df = pd.read_csv(ROOT_DIR / "data" / "historical" / "nifty_90days.csv")
    # Take the last 15,000 1-minute bars (~40 active trading sessions)
    recent_df = raw_df.tail(15000).copy()
    run_adx_comparison(recent_df, "NIFTY 90-Days Recent 15,000 1-Min Bars (~40 Sessions)")
