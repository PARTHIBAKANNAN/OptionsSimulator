"""
BANKNIFTY 5-Minute In-The-Money (ITM ~Rs.500) Strategy Suite
============================================================
Top-performing 6 strategies (3 CE & 3 PE) executing on 5-minute candle closes
with 1H 50-EMA trend alignment, ITM strike selection (~Rs.500 target premium),
lot size = 30, strike step = 100, and re-entry cooldown protection.
"""
...
BANKNIFTY_TARGET_PREMIUM = 500.0


class BankNiftySupportBounce5MITM(BaseStrategy):
    """5-min pullback to 20-EMA in 1H uptrend with strong bullish close (CE)."""
    def __init__(self):
        super().__init__(
            name="BANKNIFTY_SUPPORT_BOUNCE_5M_ITM",
            direction="CE",
            strike_step=BANKNIFTY_STRIKE_STEP,
            underlying="BANKNIFTY",
            strike_mode="ITM",
            target_premium=BANKNIFTY_TARGET_PREMIUM,
    ...
    def evaluate(self, data_state: dict) -> Optional[Signal]:
        ...
        if prev.low <= ema20_5m and current.close > ema20_5m and current.close > ema50_1h and closes_strong and volume_confirmed:
            ...
            return Signal(
                ...
                confidence=0.85, rationale=f"5m BANKNIFTY support bounce off 20-EMA ({ema20_5m:.1f}) in 1H uptrend",
                entry_price=price, timestamp=ts, underlying=self.underlying,
            )
        return None


class BankNiftyHeikinAshiBullish5MITM(BaseStrategy):
    """5-min Heikin-Ashi continuation: 2 consecutive green HA candles with flat bottoms in 1H uptrend (CE)."""
    def __init__(self):
        super().__init__(
            name="BANKNIFTY_HEIKIN_ASHI_BULLISH_5M_ITM",
            direction="CE",
            strike_step=BANKNIFTY_STRIKE_STEP,
            underlying="BANKNIFTY",
            strike_mode="ITM",
...
class BankNiftyORBBullish5MITM(BaseStrategy):
    ...
    def evaluate(self, data_state: dict) -> Optional[Signal]:
        ...
        if crossed_now and volume_confirmed and (ema50 is not None and current.close > ema50):
            ...
            return Signal(
                strategy=self.name, direction="CE", action="BUY", strike=symbol,
                confidence=0.85, rationale=f"5m BANKNIFTY ORB breakout above morning high {self._range_high:.1f} on volume",
                entry_price=price, timestamp=ts, underlying=self.underlying,
            )
        return None


class BankNiftyResistanceRejection5MITM(BaseStrategy):
    """5-min test of 20-EMA from below with strong rejection close in 1H downtrend (PE)."""
    def __init__(self):
        super().__init__(
            name="BANKNIFTY_RESISTANCE_REJECTION_5M_ITM",
            direction="PE",
            strike_step=BANKNIFTY_STRIKE_STEP,
            underlying="BANKNIFTY",
            strike_mode="ITM",
    ...
    def evaluate(self, data_state: dict) -> Optional[Signal]:
        ...
        if prev.high >= ema20_5m and current.close < ema20_5m and current.close < ema50_1h and closes_weak and volume_confirmed:
            ...
            return Signal(
                ...
                confidence=0.85, rationale=f"5m BANKNIFTY resistance rejection off 20-EMA ({ema20_5m:.1f}) in 1H downtrend",
                entry_price=price, timestamp=ts, underlying=self.underlying,
            )
        return None


class BankNiftyHeikinAshiBearish5MITM(BaseStrategy):
    """5-min Heikin-Ashi continuation: 2 consecutive red HA candles with flat tops in 1H downtrend (PE)."""
    def __init__(self):
        super().__init__(
            name="BANKNIFTY_HEIKIN_ASHI_BEARISH_5M_ITM",
            direction="PE",
            strike_step=BANKNIFTY_STRIKE_STEP,
            underlying="BANKNIFTY",
            strike_mode="ITM",
    ...
    def evaluate(self, data_state: dict) -> Optional[Signal]:
        ...
        return Signal(
            strategy=self.name, direction="PE", action="BUY", strike=symbol,
            confidence=0.80, rationale="5m BANKNIFTY Heikin-Ashi bearish continuation below 1H 50-EMA",
            entry_price=price, timestamp=ts, underlying=self.underlying,
        )


class BankNiftyORBBearish5MITM(BaseStrategy):
    """5-min Opening Range Breakdown below 09:30 range low with 1H 50-EMA confirmation (PE)."""
    def __init__(self):
        super().__init__(
            name="BANKNIFTY_ORB_BEARISH_5M_ITM",
            direction="PE",
            strike_step=BANKNIFTY_STRIKE_STEP,
            underlying="BANKNIFTY",
            strike_mode="ITM",
    ...
    def evaluate(self, data_state: dict) -> Optional[Signal]:
        ...
        if crossed_now and volume_confirmed and (ema50 is not None and current.close < ema50):
            ...
            return Signal(
                strategy=self.name, direction="PE", action="BUY", strike=symbol,
                confidence=0.85, rationale=f"5m BANKNIFTY ORB breakdown below morning low {self._range_low:.1f} on volume",
                entry_price=price, timestamp=ts, underlying=self.underlying,
            )
        return None
